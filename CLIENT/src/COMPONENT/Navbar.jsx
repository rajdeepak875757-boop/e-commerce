      ...
      <div className='h-[8vh] z-30 sticky top-0'>
        ...
        <div className='flex justify-between py-3 px-4 bg-white/45 backdrop-blur-lg border-b border-black/35 shadow-md shadow-black '>
           ...
           <div className=' w-[30vw]'>
              <ul className=' flex justify-between text-[19px] capitalize font-semibold'>
           ...
           <div className='flex gap-5 items-center'>
            <input type="search" placeholder='what are you looking for ?' className='capitalize bg-black/20 px-3 py-2 rounded-sm' />
            <Link to={'/cart'}><FaShoppingCart className='text-3xl'/> <p className='bg-red-600 px-2 py-1 text-[12px] rounded-full text-white absolute top-2 right-13'>{cart.items.length}</p></Link>
            {user.isAuthenticated ? <div className='h-8 w-8 bg-red-500 rounded-full'></div>:<CgProfile onClick={()=>setprofile(!profile)} className='text-3xl'/>}
            ...
            {user.isAuthenticated && <CgProfile onClick={()=>setprofile(!profile)} className='text-3xl'/>}
            {profile && user.isAuthenticated ? <div className='absolute backdrop-blur-lg px-5 py-5 bg-black/60 right-5 top-10 rounded-md leading-12 font-semibold text-[18px] text-white' onClick={()=>setprofile(false)}> 
